...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { type = 'tictactoe', language, category, username } = req.body as { type?: string; language?: string; category?: string; username?: string };
  ...
  const state = type === 'hangman'
    ? createHangmanState(playerToken, language as 'en' | 'el', category as any, username)
    : createTicTacToeState(playerToken, username);
